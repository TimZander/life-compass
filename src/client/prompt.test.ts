...
describe("material an ask sends the reader to", () => {
  it("promptFor_AnAskLinkingToAPage_CarriesWhatItPointsAt", () => {
    // Arrange — the defect this exists for, found on a device (#96). Day 2's first item says
    // "Open the [values starter list](../reference/values-list.md) and circle every word that
    // feels like you", and the prompt carried that sentence and nothing else: an assistant was
    // handed a relative link it cannot follow, since the application never reaches the network
    // (0006), and told to work from a list it could not see.
    const ask = ASKS["day2.brainstorm"] ?? "";
    assert.ok(ask.includes("values-list.md"), "the fixture ask no longer links to the list");

    // Act
    const text = textFor("day2.brainstorm");

    // Assert — the words themselves, not a reference to them.
    assert.ok(text.includes("Adventure · Authenticity"), "the list did not travel with the ask");
    assert.ok(text.includes("Wonder"), "only part of the list travelled");
    assert.match(text, /^### The list this refers to$/m, "the list is not introduced");
    assert.match(text, /nothing to open/i, "nothing tells the assistant it already has it");
  });

  it("promptFor_TheListItCarries_IsOfferedRatherThanReadOut", () => {
    // Arrange — 69 words handed to an assistant with no instruction is 69 words read aloud at
    // somebody mid-dictation, which is the wall of text 0001 exists to prevent. And the
    // worksheet says "Add your own", so a list presented as the whole of the answer would
    // narrow the exercise rather than serve it.
    // Act
    const text = textFor("day2.brainstorm");

    // Assert
    assert.match(text, /a few at a time/i, "the assistant may read the whole list out");
    assert.match(text, /do not limit me to it/i, "the list is presented as the only answers");
  });

  it("promptFor_AQuestionWhoseAskLinksNowhere_CarriesNoMaterial", () => {
    // Arrange — negative case, and the reason the material is keyed per question rather than
    // shipped with every prompt. Only one ask in the workbook points at that page; the other
    // two links are cross-references, and carrying a whole worksheet into a prompt for "add
    // your own values" is the failure this was written to prevent.
    //
    // `values.additions` is the sharpest version of it: it SITS on the list's page, so #91
    // hands it the words as its own ask and it needs nothing carried. A rule that keyed on the
    // page rather than on the link would give it the list a second time.
    const ask = ASKS["values.additions"] ?? "";
    assert.ok(ask.includes("Adventure · Authenticity"), "the fixture no longer carries the list in its ask");

    // Act
    const text = textFor("values.additions");

    // Assert — the words are there because the page put them there, and nothing was added.
    assert.ok(!text.includes("The list this refers to"), "material travelled with a cross-reference");
    assert.equal(text.split("Adventure · Authenticity").length - 1, 1, "the list was carried in twice");
  });

  it("promptFor_MaterialInAnItemOfSeveralQuestions_SitsUnderItsOwnQuestion", () => {
    // Arrange — the material belongs to ONE question, so in an item covering several it has to
    // nest under that question's heading rather than float at the item's level, for the reason
    // `priorSection` takes a heading depth: a reader and an assistant both resolve what belongs
    // to what by how deeply it sits.
    // Act
    const text = itemText(
      "1. Brainstorm from the list (15 min)",
      { group: "day2.brainstorm" },
      { group: "day2.shortlist_ten" },
    );

    // Assert
    assert.match(text, /^#### The list this refers to$/m, "the material is not nested under its question");
    const [, first = "", second = ""] = text.split(/^### Question \d+ of \d+ — `[^`]+`$/gm);
    assert.ok(first.includes("Adventure · Authenticity"), "the list is not under the question that asks for it");
    assert.ok(!second.includes("Adventure · Authenticity"), "the list travelled under another question too");
  });
});
